...
from marketml.config import defaults as D
...
def build_config_dict(form) -> tuple[dict, list[str]]:
    ...
    if not target_symbol:
        errors.append("« Symbole cible » est requis.")
    ...
    config_dict = {
        ...
        "objective": {
            ...
            "target_source": form.get("target_source", "yfinance"),
            "horizons": horizons,
            "flat_thr": flat_thr,
            "regimes": regimes,
        },
        "universe": {
            "yf_tickers": _split_list(form.get("yf_tickers", "")),
            "fred_series": _kv_lines(form.get("fred_series", "")),
...
def to_view(cfg: dict) -> dict:
    ...
    return {
        ...
        "target_source": obj.get("target_source", "yfinance"),
        "horizons": ",".join(str(h) for h in obj.get("horizons", [])),
        "flat_thr": obj.get("flat_thr", D.DEFAULT_FLAT_THR),
        "regimes": ",".join(obj.get("regimes", [])),
        "yf_tickers": ", ".join(uni.get("yf_tickers", [])),
        "fred_series": "\n".join(f"{k}: {v}" for k, v in uni.get("fred_series", {}).items()),
